    ...
    def _generate_matrix_from_seed(self, rho, transpose=False, is_ntt=False):
        ...
        for i in range(self.k):
            ...
            for j in range(self.k):
                if transpose:
                    input_bytes = self._xof(rho, bytes([i]), bytes([j]), 3*self.R.n) # Generate a ninput bytes
                else:
                    input_bytes = self._xof(rho, bytes([j]), bytes([i]), 3*self.R.n) # Generate a ninput bytes
